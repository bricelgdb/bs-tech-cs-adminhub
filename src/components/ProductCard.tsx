...
export function ProductCard({ product, onClick }: ProductCardProps) {
  ...
  return (
    <div
      onClick={onClick}
      className={`group cursor-pointer rounded-lg border border-border bg-card transition-colors hover:border-border-hover ${borderClass} border-t-2`}
    >
      <div className="p-5">
        ...
        <h3 className="mt-3 text-sm font-bold text-foreground group-hover:text-primary transition-colors">{product.name}</h3>
        ...
        <div className="mt-4 grid grid-cols-3 gap-3 border-t border-border pt-3">
          <div>
            ...
            <div className="text-sm font-bold text-foreground">{product.seats}</div>
          </div>
          <div>
            <div className="text-[10px] text-muted-foreground">Utilisation</div>
            <div className="text-sm font-bold text-foreground">{product.utilisation}%</div>
          </div>
          <div>
            <div className="text-[10px] text-muted-foreground">Cost/mo</div>
            <div className="text-sm font-bold text-foreground">{product.costMonthly > 0 ? `€${product.costMonthly.toLocaleString()}` : "Internal"}</div>
          </div>
        </div>
      </div>
    </div>
